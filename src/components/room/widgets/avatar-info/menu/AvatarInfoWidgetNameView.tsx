...
export const AvatarInfoWidgetNameView: FC<AvatarInfoWidgetNameViewProps> = props =>
{
    ...
    {
        const newClassNames: string[] = [ "flex items-center !min-w-fit !p-[6px_15px_8px] text-[#636363] dark:text-white [text-shadow:_0_1px_0_#fff] dark:[text-shadow:_0_1px_0_#33312B]" ]
        ...
        <ContextMenuView objectId={ nameInfo.roomIndex } category={ nameInfo.category } userType={ nameInfo.userType } fades={ (nameInfo.id !== GetSessionDataManager().userId) } classNames={ getClassNames } onClose={ onClose }>
            <div className="flex items-center gap-1">
                {nameInfo.relationshipStatus !== 0 &&
                    <i className={`h-3.5 w-[13px] bg-[url('/client-assets/images/spritesheet.png')] dark:bg-[url('/client-assets/images/spritesheet-dark.png')] ${ getUserRelationshipPosition() }`} /> }

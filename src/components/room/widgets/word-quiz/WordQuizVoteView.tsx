    ...
    return (
        <ObjectLocationView objectId={ userIndex } category={ RoomObjectCategory.UNIT } { ...rest }>
            <div className={ `flex size-8 items-center justify-center ${ (vote === VALUE_KEY_DISLIKE) ? "illumina-poll-dislike" : "illumina-poll-like" }` }>
                <i className={ `bg-[url('/client-assets/images/spritesheet.png')] dark:bg-[url('/client-assets/images/spritesheet-dark.png')] ${ (vote === VALUE_KEY_DISLIKE) ? "size-[22px] bg-[-444px_-292px]" : "h-[21px] w-[18px] bg-[-393px_-292px]" }` } />
            </div>
        </ObjectLocationView>
    )
}

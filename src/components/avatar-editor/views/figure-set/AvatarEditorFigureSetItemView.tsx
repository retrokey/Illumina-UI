...
export const AvatarEditorFigureSetItemView: FC<AvatarEditorFigureSetItemViewProps> = props =>
{
    ...
    return (
        <LayoutGridItem className="!size-[52px]" itemImage={ (partItem.isClear ? undefined : partItem.imageUrl) } itemAbsolute={ true } itemActive={ partItem.isSelected } { ...rest }>
            { !hcDisabled && partItem.isHC && <i className="absolute bottom-1 right-1 h-[9px] w-2.5 bg-[url('/client-assets/images/spritesheet.png')] dark:bg-[url('/client-assets/images/spritesheet-dark.png')] bg-[-298px_-69px]" /> }
            { partItem.isClear && <div className="size-[27px] bg-[url('/client-assets/images/avatar-editor/spritesheet.png')] dark:bg-[url('/client-assets/images/avatar-editor/spritesheet-dark.png')] bg-[-329px_-43px]" /> }
            { children }
        </LayoutGridItem>
    )
}

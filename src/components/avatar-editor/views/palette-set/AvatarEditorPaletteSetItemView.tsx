...
export const AvatarEditorPaletteSetItem: FC<AvatarEditorPaletteSetItemProps> = props =>
{
    ...
    return (
        <div className={`relative h-[17px] w-[13px] cursor-pointer ${colorItem.isSelected ? "-translate-y-0.5" : null}`} { ...rest }>
            <div className={`absolute left-0 top-0 size-full bg-[url('/client-assets/images/avatar-editor/spritesheet.png')] dark:bg-[url('/client-assets/images/avatar-editor/spritesheet-dark.png')] ${colorItem.isSelected ? "bg-[-329px_-24px]" : "bg-[-343px_-24px]"}`} />
            <div className="size-full" style={{ backgroundColor: colorItem.color }} />
            { (!hcDisabled && colorItem.isHC) && <i className="absolute bottom-0 right-0 h-[9px] w-2.5 bg-[url('/client-assets/images/spritesheet.png')] dark:bg-[url('/client-assets/images/spritesheet-dark.png')] bg-[-298px_-69px]" /> }
        </div>
    )
}

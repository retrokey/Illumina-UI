                                ...
                                <div key={index} className="mb-0.5 flex justify-between">
                                    <div onClick={event => setSelectedIndex(index)}>
                                        {(badgeParts[index].code && badgeParts[index].code.length > 0) ? (
                                            ...
                                            <Button className="!size-[49px]">
                                                <i className="size-[17px] bg-[url('/client-assets/images/groups/spritesheet.png')] dark:bg-[url('/client-assets/images/groups/spritesheet-dark.png')] bg-[-455px_-124px]" />
                                            </Button>
                                        )}
                                    </div>
                                    <div className="grid grid-cols-3 gap-0">
                                        {POSITIONS.map((position, posIndex) => (
                                            <i key={posIndex} className={`size-[15px] bg-[url('/client-assets/images/groups/spritesheet.png')] dark:bg-[url('/client-assets/images/groups/spritesheet-dark.png')] ${badgeParts[index].position === position ? "bg-[-439px_-124px]" : "bg-[-423px_-124px]"}`} onClick={event => setPartProperty(index, "position", position)} />
                                        ))}
                                    </div>
                                    <div className="grid grid-cols-8 content-start">
                                        {groupCustomize.badgePartColors.length > 0 && groupCustomize.badgePartColors.map((item, colorIndex) => (
                                            <div key={colorIndex} className={`size-[15px] bg-[url('/client-assets/images/groups/spritesheet.png')] dark:bg-[url('/client-assets/images/groups/spritesheet-dark.png')] ${badgeParts[index].color === colorIndex + 1 ? "bg-[-407px_-124px]" : "bg-[-391px_-124px]"}`} style={{ backgroundColor: "#" + item.color }} onClick={event => setPartProperty(index, "color", colorIndex + 1)} />
                                        ))}
                                    </div>
                                </div>
                            )
                ...
                <div className="illumina-scrollbar grid max-h-[300px] grid-cols-5">
                    {badgeParts[selectedIndex].type === GroupBadgePart.SYMBOL && (
                        <div className="illumina-furni-item flex size-[41px] items-center justify-center overflow-hidden" onClick={event => setPartProperty(selectedIndex, "key", 0)}>
                            <i className="size-[27px] bg-[url('/client-assets/images/groups/spritesheet.png')] dark:bg-[url('/client-assets/images/groups/spritesheet-dark.png')] bg-[-473px_-124px]" />
                        </div>
                    )}
